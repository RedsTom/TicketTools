...
public class TicketCommand implements ICommand {
  ...
  private void runWithSelectedUser(MessageReceivedEvent event, String[] args)
    ...
    try {
      ...
      embeds
          .errorMessage(String.format("L'identifiant `%s` est invalide.", userId))
          .sendMessage(event.getChannel())
          .queue();
      return;
    }
    ...
    if (user == null) {
      embeds
          .errorMessage(
              String.format("`Le membre avec l'identifiant `%s` ne peut être trouvé.", userId))
          .sendMessage(event.getChannel())
          .queue();
      return;
    }
    ...
    embeds
        .successMessage(String.format("Le ticket avec %s a bien été ouvert.", user.getName()))
        .sendMessage(event.getChannel())
        .queue();
  }
